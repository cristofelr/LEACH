...
//code is changed to calculate threshold properly and identifying dead node
//Authors:: Abhishek Singh(abhishek.singh199418@gmail.com) and Wuxiucheng Wang at University of Rochester 
//Rights reserved to both authors and University of Rochester
...
int BASE_STATION_X_DEFAULT = 500;

int BASE_STATION_Y_DEFAULT = 500;  
		...
		// transmit  
			if(network_DIRECT[i].bCurrent > 0){  
				...
				//	cout << "hi " << endl;
					failed_transmission++;  
					if(flag){
						//cout << "power_consumption: " << power_consumption << " network_DIRECT[i].bCurrent: " << network_DIRECT[i].bCurrent << endl;
						//cout << "Directdead_node's round: " << round << endl;
						flag = 0;
					}
				    }
			...
			//if(round == 1000 ) cout  << "network_pwr: " << averageEnergy(network_DIRECT) << endl; 
			dir.push_back(averageEnergy(network_DIRECT));
			round += 1;

	
			//vec_WriteFile(dir); 
//			std::ofstream output_file("./directsimulation.txt");
//    			std::ostream_iterator<std::string> output_iterator(output_file, "\n");
//    			std::copy(dir.begin(), dir.end(), output_iterator);

}
	ofstream stats;
	//stats << "direct_simulation" << endl;
	stats.open("direct_simulation.txt");
...
network_DIRECT_NPP = (struct sensor *) malloc(60 * sizeof(struct sensor));  
...
//                 run for  
    ...
    if((fp = fopen(filename, "r")) == NULL){  
        return (struct sensor *) malloc(60 *   
    ...
    return (struct sensor *) malloc(60 * sizeof(struct sensor));   
...
int runmodification(const struct sensor network[]){
struct sensor * network_LEACH;      // wireless sensor network to run sim on  
...
//printf("\nRunning the Modified Transmission Simulation \n");  
// our iterating loop runs of total rounds, this is   
// the expected lifetime of the network  
...
//            if(network_LEACH[i].bCurrent > 0.0){  
  ...
  //              network_LEACH[i].head = DEAD_NODE;
    ...
    ofstream d_stats;
    d_stats.open("modified_simulation.txt");
    //d_stats << "leach_simulation" << endl;
    for(unsigned int i = 0; i < dir.size(); i++) d_stats << dir[i] << endl;//d_stats << dir[i] << endl ;
